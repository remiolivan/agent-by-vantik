...
export default function Calendar() {
  ...
  const [viewingEvent, setViewingEvent] = useState(null) // read-only view before entering edit mode
  ...
  return (
    ...
    >
      ...
      {view === 'day' && (
        <DayView date={cursor} events={eventsOn(cursor)} onEventClick={openViewEvent} onSlotClick={openNewEvent} />
      )}
      {view === 'week' && (
        <WeekView days={weekDays} eventsOn={eventsOn} onEventClick={openViewEvent} onSlotClick={openNewEvent} />
      )}
      {view === 'month' && (
      ...
      {viewingEvent && (
        <EventView
          ...
          onEdit={() => openEditEvent(viewingEvent)}
          onDelete={() => { setViewingEvent(null); deleteEvent(viewingEvent) }}
        />
      )}
...
function EventView({ ev, onClose, onEdit, onDelete }) {
  const syncedProviders = (ev.calendar_event_syncs || []).filter((s) => s.sync_status === 'synced').map((s) => s.calendar_connections?.provider)
  return (
      ...
      <div className="absolute inset-x-0 bottom-0 sm:inset-y-0 sm:right-0 sm:left-auto sm:w-full sm:max-w-md bg-white rounded-t-2xl sm:rounded-none max-h-[90vh] sm:max-h-none overflow-y-auto">
        ...
        <div className="p-5 space-y-4">
          <div className="text-sm text-ink">
            {new Date(ev.start_at).toLocaleString([], { weekday: 'long', month: 'long', day: 'numeric' })}
          </div>
