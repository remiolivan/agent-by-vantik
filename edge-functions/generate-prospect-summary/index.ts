...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    page.drawText("Made with Agent by Vantik", { x: 50, y: 30, size: 8, font, color: fog });
    ...
    // Short share link (see the `share` function): resolves to a freshly
    // signed URL at click time, so the link we hand out is short and stable.
