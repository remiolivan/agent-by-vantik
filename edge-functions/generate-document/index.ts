...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { propertyId, prospectId, title, items, notes, dueDate, taxRate, currency, documentId, recipientName, recipientEmail, recipientAddress } = await req.json();
    ...
    // Editing an existing invoice: reuse its invoice number, and we'll UPDATE
    // the row (with a freshly regenerated PDF) instead of inserting a new one.
    let existingDoc: any = null;
    if (documentId) {
      const { data } = await adminClient.from("documents").select("*").eq("id", documentId).eq("org_id", membership.org_id).maybeSingle();
      existingDoc = data;
      if (!existingDoc) {
    ...
    if (existingDoc) {
      ...
      invoiceNumber = `INV-${String((existingCount ?? 0) + 1).padStart(4, "0")}`;
    }

    // The recipient (destinataire) can be typed in directly, or fall back to
    // the linked prospect/property when not provided.
    let billToName: string | null = recipientName || null;
    let billToAddress: string | null = recipientAddress || null;
    if (!billToName && prospectId) {
    ...
    if (propertyId) {
      ...
      if (!billToAddress) billToAddress = property?.address ?? null;
    }
    ...
    if (billToName || propertyTitle) {
      ...
      if (recipientEmail) { page.drawText(recipientEmail, { x: 50, y, size: 9, font, color: fog }); y -= 13; }
    ...
    let docRow;
    if (existingDoc) {
      const { data, error: updateErr } = await adminClient.from("documents").update(rowPayload).eq("id", existingDoc.id).select().single();
      if (updateErr) {
        return new Response(JSON.stringify({ error: updateErr.message }), { status: 500, headers: { ...CORS, "Content-Type": "application/json" } });
      }
      docRow = data;
      // Best-effort cleanup of the previous PDF now that the new one is live.
      if (existingDoc.file_url && existingDoc.file_url !== path) {
        await adminClient.storage.from("property-documents").remove([existingDoc.file_url]);
      }
    } else {
      const { data, error: insertErr } = await adminClient.from("documents").insert({
        ...rowPayload,
