...
export default function Documents() {
  ...
  return (
    <Layout
      ...
      action={
        <div className="flex items-center gap-2">
          <button onClick={() => setShowSettings(true)} className="text-navyDeep border border-navyDeep/30 rounded-md p-2.5" aria-label="Invoice settings">
            <Settings size={16} />
          </button>
          <button onClick={openNew} className="bg-amber text-ink font-bold rounded-md px-4 py-2.5 lg:py-2 whitespace-nowrap">
            + New invoice
          </button>
        </div>
      }
    >
      {showForm && (
        <form onSubmit={generate} className="px-4 py-6 -mx-4 mb-6 bg-white border-y border-border sm:mx-0 sm:rounded-md sm:border max-w-2xl space-y-3">
          <input
            value={invoice.title} onChange={(e) => setInvoice({ ...invoice, title: e.target.value })}
            placeholder="Title (e.g. Commission invoice)" className="w-full border border-border rounded-md px-3 py-2.5 text-sm"
          />
          ...
          <div>
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <input
                ...
                placeholder="Name (defaults to linked prospect)" className="flex-1 border border-border rounded-md px-3 py-2.5 text-sm"
              />
              <input
                value={invoice.recipientEmail} onChange={(e) => setInvoice({ ...invoice, recipientEmail: e.target.value })}
                placeholder="Email" type="email" className="flex-1 border border-border rounded-md px-3 py-2.5 text-sm"
              />
            </div>
            <input
              value={invoice.recipientAddress} onChange={(e) => setInvoice({ ...invoice, recipientAddress: e.target.value })}
              placeholder="Address (defaults to linked property)" className="w-full mt-3 border border-border rounded-md px-3 py-2.5 text-sm"
            />
          </div>

          <div className="flex flex-col sm:flex-row gap-3">
            <select value={invoice.prospectId} onChange={(e) => setInvoice({ ...invoice, prospectId: e.target.value })} className="flex-1 border border-border rounded-md px-3 py-2.5 text-sm">
              <option value="">No linked prospect</option>
              {prospects.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}
            </select>
            <select value={invoice.propertyId} onChange={(e) => setInvoice({ ...invoice, propertyId: e.target.value })} className="flex-1 border border-border rounded-md px-3 py-2.5 text-sm">
          ...
          {invoice.items.map((item, idx) => (
            <div key={idx} className="flex gap-3">
              <input
                ...
                placeholder="Description" className="flex-1 border border-border rounded-md px-3 py-2.5 text-sm"
              />
              <NumberInput
                value={item.amount} onChange={(v) => updateItem(idx, 'amount', v)}
                placeholder="Amount" className="w-24 sm:w-32 border border-border rounded-md px-3 py-2.5 text-sm"
              />
            </div>
          ))}
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <div className="w-28">
              ...
              <input
                ...
                placeholder="0" type="number" className="w-full border border-border rounded-md px-3 py-2.5 text-sm"
              />
            </div>
            <div className="w-24">
          ...
          <div className="flex gap-3">
            <button type="submit" disabled={generating} className="bg-mid text-white text-sm font-medium rounded-md px-4 py-2.5 disabled:opacity-50 flex-1 sm:flex-none">
      ...
      <div className="max-w-2xl">
        ...
        <div className="space-y-2 sm:hidden">
          {documents.map((d) => (
            <div key={d.id} className="w-full bg-white border border-border rounded-md p-4 flex items-center justify-between gap-2">
      ...
      <div className="absolute inset-y-0 right-0 w-full sm:max-w-md bg-paper overflow-y-auto">
        ...
        <form onSubmit={save} className="px-5 py-5 space-y-3">
          ...
          <input
            ...
            placeholder="Business name" className="w-full border border-border rounded-md px-3 py-2.5 text-sm"
          />
          <textarea
            value={form.invoice_address} onChange={(e) => setForm({ ...form, invoice_address: e.target.value })}
            placeholder="Address" rows={2} className="w-full border border-border rounded-md px-3 py-2.5 text-sm"
          />
          <input
            value={form.invoice_email} onChange={(e) => setForm({ ...form, invoice_email: e.target.value })}
            placeholder="Email" className="w-full border border-border rounded-md px-3 py-2.5 text-sm"
          />
          <input
          ...
          <input
            ...
            placeholder="IBAN" className="w-full border border-border rounded-md px-3 py-2.5 text-sm"
          />
          <input
            value={form.invoice_trn} onChange={(e) => setForm({ ...form, invoice_trn: e.target.value })}
            placeholder="TRN (tax registration number)" className="w-full border border-border rounded-md px-3 py-2.5 text-sm"
          />
          <div>
            ...
            <input
              ...
              className="w-32 border border-border rounded-md px-3 py-2.5 text-sm"
            />
          </div>
          ...
          <div>
            ...
            <button
              ...
              className="flex items-center gap-1.5 text-sm text-navyDeep border border-navyDeep/30 rounded-md px-3.5 py-2 disabled:opacity-50"
            >
