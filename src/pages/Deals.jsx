...
export default function Deals() {
  ...
  return (
    <div className="min-h-screen bg-paper">
      ...
      {showNew && (
        <form onSubmit={createDeal} className="flex gap-3 items-center px-8 py-4 bg-white border-b border-fog/20">
          <input
            value={newTitle} onChange={(e) => setNewTitle(e.target.value)}
            placeholder="Deal title" required
            className="border border-fog/30 rounded px-3 py-2 text-sm flex-1"
          />
          <input
            value={newValue} onChange={(e) => setNewValue(e.target.value)}
            placeholder="Value" type="number"
            className="border border-fog/30 rounded px-3 py-2 text-sm w-32"
          />
        ...
        <div className="flex gap-4 min-w-max">
          {stages.map((stage) => (
            <div key={stage.id} className="w-64 flex-shrink-0">
              <div className="font-mono text-xs uppercase tracking-wide text-fog mb-3 flex items-center justify-between">
                <span>{stage.name}</span>
                <span>{deals.filter((d) => d.stage_id === stage.id).length}</span>
              </div>
              <div className="space-y-3">
                {deals.filter((d) => d.stage_id === stage.id).map((deal) => (
                  <div key={deal.id} className="bg-white border border-fog/20 rounded p-4">
                    <div className="text-sm font-medium text-ink mb-1">{deal.title}</div>
                    {deal.value && (
                      <div className="font-mono text-xs text-fog mb-3">
